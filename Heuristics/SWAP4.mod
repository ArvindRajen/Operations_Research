...
subject to SWAP4: fp[3] + fp[13] + fp[19] + fp[22]=3;

subject to PFOFF: fp[1] + fp[2] + sum{i in 4..12}fp[i] + sum{i in 14..18}fp[i] + fp[20] + fp[21] + sum{i in 23..25}fp[i]=1;

subject to ADD5{q in Q}: fw[25,q]=1;

subject to ADD6{q in Q}: fw[24,q]=1;

subject to ADD7{q in Q}: fw[23,q]=1;

subject to WHZEROS{q in Q}: sum{i in 1..22}fw[i,q]=0;
