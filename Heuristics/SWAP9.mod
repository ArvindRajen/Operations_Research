...
subject to PFFIX: fp[3] + fp[13] + fp[19] + fp[22]=4;

subject to PFZEROS: fp[1] + fp[2] + sum{i in 4..12}fp[i] + sum{i in 14..18}fp[i] + fp[20] + fp[21] + sum{i in 23..25}fp[i]=0;

subject to SWAP6: fw[11,1] + sum{i in 24..25}fw[i,1]=3;

subject to SWAP7: sum{i in 23..25}fw[i,2]=3;

subject to SWAP8: sum{i in 23..25}fw[i,3]=3;

subject to SWAP9: sum{i in 23..25}fw[i,4]=2;

# subject to QUARTER: sum{i in 23..25}fw[i,4]=3;

# subject to QUARTEROFF: sum{i in 1..22} fw[i,4]=0;
